...
function RemoveOffsetsFromStyleData() {
	for (var i = 0; i < singleLettersCount; i++) {
		...
		comboData.width = width + 500; //TODO: Remove " + 100"
	...
	for (var i = singleLettersCount; i < styleData.length; i++) {
		...
		//Add extra properties to combo data
		comboData.width = width + 500; //TODO: Remove " + 100"
		comboData.firstLetterEnd = letterJoinPos;
...
function ValidateStyleData() {
	for (var i = 0; i < styleData.length; i++) {
		...
		if (styleData[i].strokeData === undefined || styleData[i].strokeData.length === 0) {
...
function GenerateHandwriting(textInput) {
	...
	var words = []; //Just used for logging
	
	//for (var i = 0; i < textInputData.length; i++) {
	slowLoop(0, textInputData.length, 1, 100, function(i) {
		...
		logIndex++;
		console.log("@" + new Date().getMinutes() + ":" + new Date().getSeconds() + " : " + logIndex);
		
		
		
		// --- Apply word to canvas ---
		
		let wordPointArray = AddPointToAllInPointArray(wordData.pointArray, new Point(0, vPadding));
		let wordWidth = wordData.width;
		...
		canvas.height = ((600 + 2*vPadding) / 10).toString();
...
function CleanUpTextInput(textInput) {
	var res = textInput.toString();
	var res = res.replace(/\s/gm, " "); //Replace enters, tabs, etc with spaces
	var res = res.replace(/[^0-9a-z\~\!\@\#\$\%\^\&\)\(\_\+\=\:\;\'\"\,\.\<\>\/\?\£\ \-]/gmi, "") //Remove all characters other than those listed (in /[^here]/gmi) (global, multiline, case insensitive (hence 'gmi'))
...
function InterpretTextInput(textInput) {
	...
	for (var i = 0; i < wordsData.length; i++) {
		...
		for (var j = 0; j < lettersData.length; j++) {
			//Setup
			let letter1 = lettersData[j];
			let letter2 = lettersData[j + 1] || "*"; //TODO?: Replace "*"
			let combo = letter1 + "-" + letter2;
			let layer      = ( layerBool && "layer2") || "layer1";
			let otherLayer = (!layerBool && "layer2") || "layer1";
			//Push to arrays, depending on stuff
			if (combinations.containsPrimitive(combo)) {
				combinedLettersData[layer].push(letter1, letter2);
				layerBool = !layerBool;
			} else {
				if (combinedLettersData[otherLayer][j] === " ") {
					combinedLettersData[layer].push(letter1, " ");
					layerBool = !layerBool;
				} else {
					combinedLettersData[     layer].push(" ");
					combinedLettersData[otherLayer].push(" ");
					//[Stay in same layer]
				}
			}
		...
		//console.log(combinedLettersData);
...
function GenerateWord(twoLayerWordData) {
	...
	if (len === 0) {
		...
		let pointArray = AddPointToAllInPointArray(styleData[letterIndex].strokeData, new Point(hPadding, 0));
		
		//Apply
		let result = {pointArray:[], width:undefined};
		result.pointArray = pointArray;
		result.width = letterWidth + 2*hPadding;
		...
		for (var i = 0; i < len;) {
			...
			if (word[layer][i] === " ") {
				...
				//If condition met, swap the layers and try again
				if (layer === "layer2" && word.layer1[i] === " ") {
					let temp = word.layer1;
					word.layer1 = word.layer2;
					word.layer2 = temp;
					layerBool = !layerBool;
					continue;
				}
		...
		//Get width
		let wordWidth = offsetSoFar + hPadding;
		...
		//~//	let combinedLayers = {pointArray:wordData.layer1.pointArray.concat(wordData.layer2.pointArray), width:wordWidth};
	...
	if (beforeEnd && !beyondStart) {
		return {
			start:{
				...
				dist:strengthSections[0].start.dist - 10000 //Arbitrary value (make sure it's high enough)
			},
			end:{
				fraction:strengthSections[0].start.fraction,
		...
		return {
			start:{
				...
				fraction:strengthSections[strengthSections.length - 1].end.fraction,
				dist:strengthSections[strengthSections.length - 1].end.dist + 10000 //Arbitrary value (make sure it's high enough)
			}
		};
	}
